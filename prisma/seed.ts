...
async function main() {
  ...
  console.log(`  Pessoas: ${heitor.name}, ${linard.name}, ${pedro.name}`);
  console.log(`  Produtos: ${nutria.name}, ${exomia.name}`);
  console.log(`  Feature: "${feature.title}" (${feature.status})`);
  console.log(`  Tasks: ${tasks.length}`);
  console.log(`  Decisions: 3`);
  console.log(`  Artifacts: 4`);
  console.log(`  ActivityLog: ${logs.length} eventos`);
}
